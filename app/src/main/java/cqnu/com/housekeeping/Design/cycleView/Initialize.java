...
public class Initialize {
     ...
     * 初始配置轮播页面
     */
    public void initialize(CycleViewPager cycleViewPager,List<AdvertisementItem> adlist,CycleViewPager.ImageCycleViewListener CycleViewListener,Context context) {
        views.clear();
        infos.clear();
        for(int i = 0; i < adlist.size(); i ++){
            ADInfo info = new ADInfo();
            info.setUrl(adlist.get(i).getImg());
